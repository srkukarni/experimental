...
public class AuroraScheduler implements IScheduler, IScalable {
  ...
  protected Map<AuroraField, String> createAuroraProperties(Resource containerResource) {
    ...
    auroraProperties.put(AuroraField.IS_STATEFUL, Boolean.toString(Context.isStateful(config)));

    String completeCkptmgrProcessClasspath = new StringBuilder()
        .append(Context.ckptmgrClassPath(config)).append(":")
        .append(Context.stateBackendSandboxClassPath(config))
        .toString();
    auroraProperties.put(AuroraField.SANDBOX_CKPTMGR_CLASSPATH, completeCkptmgrProcessClasspath);
