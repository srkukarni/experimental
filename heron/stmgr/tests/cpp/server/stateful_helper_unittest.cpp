...
static heron::proto::api::Topology* GenerateDummyTopology(
  ...
  kv->set_key(heron::config::TopologyConfigVars::TOPOLOGY_STATEFUL);
  kv->set_value("true");
...
void test_helper(int _ncomponents, int _ninstances) {
  ...
  std::map<int32_t, std::set<int32_t>> upstreamers;
  std::map<int32_t, std::set<int32_t>> downstreamers;
  sp_int32 task_id = 1;
  // first for spouts
  for (int i = 0; i < _ncomponents; ++i) {
    for (int j = 0; j < _ninstances; ++j) {
      upstreamers[task_id] = std::set<int32_t>();
      downstreamers[task_id] = std::set<int32_t>();
  ...
  auto helper = new heron::stmgr::StatefulHelper();
...
TEST(StatefulHelper, test_logic) {
  for (int i = 1; i < 4; ++i) {
    for (int j = 1; j < 4; ++j) {
      test_helper(i, j);
    }
