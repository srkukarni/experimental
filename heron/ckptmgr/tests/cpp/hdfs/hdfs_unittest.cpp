...
#define USER "karthikz"
#define HDFS_SUPERUSER "karthikz"

#define BASE_DIR TEST_HDFS_PREFIX"/testCInterface/"

sp_string function_config_filename;
 ...
 */
static inline bool CheckBuffer(const char * buffer, size_t size, size_t offset) {
  int64_t todo = size;
...
TEST(TestCInterfaceConnect, TestConnect_Success) {
    hdfsFS fs = NULL;
    char * uri = NULL;
    ::setenv("LIBHDFS3_CONF", function_config_filename.c_str(), 1);
...
TEST_F(TestCInterface, TestFileSeek_InvalidInput) {
    ...
    ASSERT_TRUE(file != NULL);
    EXPECT_EQ(4, hdfsWrite(fs, file, "abcd", 4));
    EXPECT_EQ(0, hdfsCloseFile(fs, file));
    ...
    EXPECT_EQ(0, hdfsCloseFile(fs, file));
    file = hdfsOpenFile(fs, BASE_DIR"/testFileSeek", O_WRONLY | O_APPEND, 0, 0, 0);
    ASSERT_TRUE(NULL != file);
...
TEST_F(TestCInterface, TestSetWorkingDirectory_Success) {
    ...
    strcat(target, "/./" BASE_DIR);  // NOLINT(runtime/printf)
    ...
    EXPECT_EQ(0, err);
    strcat(target, "testCreateDir");  // NOLINT(runtime/printf)
...
TEST_F(TestCInterface, TestListDirectory_Success) {
    ...
    for (int i = 0 ; i < numFiles ; ++i) {
        sprintf(path, "%s/File%d", BASE_DIR"/testListDirectoryDir", i);  // NOLINT(runtime/printf)
        ASSERT_TRUE(CreateFile(fs, path, 0, 0));
    }

    for (int i = 0 ; i < numDirs ; ++i) {
        sprintf(path, "%s/Dir%d", BASE_DIR"/testListDirectoryDir", i);  // NOLINT(runtime/printf)
...
TEST_F(TestCInterface, TestChmod_Success) {
    ...
    ASSERT_TRUE(info != NULL);
    // not executable
    EXPECT_EQ(info->mPermissions, 0777);
    hdfsFreeFileInfo(info, 1);
...
static void
TestHFlushAndSync(hdfsFS fs, hdfsFile file, const char * path, int64_t blockSize, bool sync) {
    hdfsFile in;
    size_t offset = 0;
...
TEST_F(TestCInterface, TestAppend2) {
    ...
    for (int i = 0; i < 100; i++) {
        LOG(INFO) << "Loop in testAppend2 " <<  i;
...
TEST_F(TestCInterface, TestGetBlockFileLocations_Success) {
    ...
    hdfsCloseFile(fs, out);
    EXPECT_TRUE(NULL !=
     (bl = hdfsGetFileBlockLocations(
       fs, BASE_DIR"/TestGetBlockFileLocations_Failure", 0, 1, &size)));
    EXPECT_EQ(0, size);
    hdfsFreeFileBlockLocations(bl, size);
    out = hdfsOpenFile(fs, BASE_DIR"/TestGetBlockFileLocations_Failure", O_WRONLY, 0, 0, 1024);
    ASSERT_TRUE(NULL != out);
    ASSERT_TRUE(buffer.size() == hdfsWrite(fs, out, &buffer[0], buffer.size()));
    ASSERT_EQ(0, hdfsSync(fs, out));
    EXPECT_TRUE(NULL !=
      (bl = hdfsGetFileBlockLocations(
        fs, BASE_DIR"/TestGetBlockFileLocations_Failure", 0, buffer.size(), &size)));
    EXPECT_EQ(2, size);
    ...
    EXPECT_TRUE(NULL !=
      (bl = hdfsGetFileBlockLocations(
        fs, BASE_DIR"/TestGetBlockFileLocations_Failure", buffer.size(), 1, &size)));
    EXPECT_EQ(0, size);
    hdfsFreeFileBlockLocations(bl, size);
    hdfsCloseFile(fs, out);
}
...
int main(int argc, char **argv) {
  ...
  if (argc > 1) {
    std::cerr << "Using function test file " << argv[1] << std::endl;
    function_config_filename = argv[1];
  }
